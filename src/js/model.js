...
// export const sortData = async function () {
//   const data = await state.search.results.map(async data => {
//     return await AJAX(`${API_URL}${data.id}?key=${KEY}`);
//   });
//   console.log(data);
//   const sortData = createRecipeObject(data);

//   console.log(sortData);
// };

export const sortResults = function (sortBy) {
  // Sort search results by cooking time
  if (sortBy === 'cookingTime') {
    state.search.results.sort((a, b) => {
      if (a.cookingTime < b.cookingTime) return -1;
      if (a.cookingTime > b.cookingTime) return 1;
      return 0;
    });
  }

  // Sort search results by number of ingredients
  if (sortBy === 'numIngredients') {
    state.search.results.sort((a, b) => {
